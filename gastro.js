...
const T = {
  de: {
    ...
    ignore: 'Ist mir egal', reactivate: 'Wieder werten', change: 'ändern', ratingsBy: 'Bewertungen von:', whole: 'ganz', skip: 'Überspringen', next: 'Weiter', allReset: 'Alles zurückgesetzt', allShort: 'Alle', avg: 'Durchschnitt aller {n} Stimmen',
  ...
  en: {
    ...
    ignore: 'Skip this', reactivate: 'Count again', change: 'change', ratingsBy: 'Ratings by:', whole: 'all of', skip: 'Skip', next: 'Continue', allReset: 'Everything reset', allShort: 'Everyone', avg: 'Average of all {n} voices',
...
function applyLangStatic() {
  ...
  document.getElementById('btn-home').title = t('allReset');
}
...
let selectedId = null, pendingShare = null;
let step = 'art'; try { step = localStorage.getItem(LS_STEP) || 'art'; } catch {} if (!['art', 'place', 'kur', 'main'].includes(step)) step = 'art';
const MINE = 'mine';
...
function save() {
  try {
    ...
    localStorage.setItem(LS_CITY, city); localStorage.setItem(LS_KREIS, JSON.stringify(kreise)); localStorage.setItem(LS_STEP, step);
  } catch {}
}
...
function drawPlace() {
  ...
  all.addEventListener('click', () => { kreise = []; step = 'kur'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); }); chips.appendChild(all);
  ...
  const nxt = document.getElementById('place-next');
  nxt.textContent = (kreise.length ? `${t('next')} · ${t('kreis')} ${[...kreise].sort((a, b) => a - b).join(', ')}` : `${t('next')} · ${t('allCity', { city })}`) + ' →';
}
document.getElementById('place-next').addEventListener('click', () => { step = 'kur'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); });
...
function drawTiles() {
  ...
  const mk = (art, label, emoji, count) => {
    ...
    b.addEventListener('click', () => { artFilter = art; step = 'place'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); });
    box.appendChild(b);
  };
...
let CX = 410, CY = 320; let R = 235, LABEL_R = R + 36;
function isMobile() { return window.innerWidth <= 600; }
function geo() {
  const m = isMobile();
  R = m ? 200 : 235; LABEL_R = R + (m ? 30 : 36); CX = m ? 340 : 410; CY = m ? 300 : 320;
  svg.setAttribute('viewBox', m ? '0 0 680 600' : '0 0 820 640');
...
// Klick auf «Gastroführer»: alle Filter raus, zurück auf Stufe 1
document.getElementById('btn-home').addEventListener('click', () => {
  artFilter = ''; kreise = []; selKur = []; selectedId = null; wish = Array(N).fill(DEFAULT_VALUE); step = 'art';
  save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); toast(t('allReset'));
});
...
// ---------- 4 Kuratoren ----------
function kname(k) { return k.id === 'gf' ? t('gfName') : k.name; }
function khandle(k) { return k.id === 'gf' ? t('gfHandle') : k.handle; }
function kbio(k) { return (lang === 'en' && k.bio_en) || k.bio || ''; }
function drawCurators() {
  const box = document.getElementById('kur-list'); box.innerHTML = '';
  const allActive = selKur.length === 0, mineActive = isMineList(), nMine = Object.keys(own).length;
  const mk = (avatar, name, sub, handle, color, active, onClick) => {
    const b = document.createElement('button'); b.className = 'kur' + (active ? ' active' : '');
    b.style.setProperty('--kc', color);
    b.innerHTML = `<span class="avatar">${avatar}</span><span><span class="kname">${esc(name)}</span><br><span class="kbio">${esc(sub)}</span></span><span class="khandle">${esc(handle)}</span>`;
    b.addEventListener('click', onClick); box.appendChild(b);
  };
  const go = v => { selKur = v; step = 'main'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  mk('★', t('allShort'), t('avg', { n: curators.length }), '', 'var(--text)', allActive, () => go([]));
  mk('♥', t('optMine'), t('mineBio'), nMine ? nMine + ' ' + t('places') : t('empty'), 'var(--own)', mineActive, () => go([MINE]));
  curators.forEach(k => mk(esc(initials(k.name)), kname(k), kbio(k), khandle(k), k.farbe || 'var(--accent)', selKur.includes(k.id), () => go([k.id])));
}
document.getElementById('kur-skip').addEventListener('click', () => { step = 'main'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); });
...
function drawList() {
  ...
  rows.forEach(({ r, s }, idx) => {
    ...
    div.addEventListener('click', e => { if (e.target.closest('a,button')) return; selectedId = selectedId === r.id ? null : r.id; render(); if (selectedId) window.scrollTo({ top: 0, behavior: 'smooth' }); });
...
<li><strong>Wo?</strong> – Kreis antippen oder ganz Zürich.</li>
<li><strong>Wem vertraue ich?</strong> – alle, deine Liste oder eine einzelne Stimme (oder überspringen).</li>
<li><strong>Spinne und Treffer</strong> – Spinne einstellen, Unwichtiges auf «egal»; direkt darunter das Ranking, das zu deinem Abend passt. Oben in der Kopfzeile siehst du jederzeit deine Wahl und kannst sie ändern.</li>
</ol>
...
<li><strong>Where?</strong> – tap a district or all of Zurich.</li>
<li><strong>Whose ratings do I trust?</strong> – everyone, your list or a single voice (or skip).</li>
<li><strong>Chart and matches</strong> – set the chart, skip what doesn't matter; right below it the ranking that fits your evening. The header always shows your choices; tap them to change.</li>
</ol>
...
document.getElementById('share-accept').addEventListener('click', () => {
  ...
  step = 'main'; pendingShare = null; document.getElementById('share-banner').classList.remove('show'); save(); render(); toast(t('shareTaken'));
});
...
function drawSummary() {
  ...
  if (step === 'art') return;
  const chip = (html, title, onClick) => { const b = document.createElement('button'); b.className = 'sum'; b.innerHTML = html; b.title = title; b.addEventListener('click', onClick); box.appendChild(b); };
  chip(`<span class="e">${artFilter ? (EMOJI[artFilter] || '🍽️') : '🎲'}</span>`, artFilter ? artName(artFilter) : t('surprise'), () => { step = 'art'; save(); render(); });
  if (step === 'place') return;
  chip(`<span class="e">📍</span>${esc(kreise.length ? `${t('kreis')} ${[...kreise].sort((x, y) => x - y).join(', ')}` : t('whole') + ' ' + city)}`, t('change'), () => { step = 'place'; save(); render(); });
  if (step === 'kur') return;
  const allActive = selKur.length === 0, mineActive = isMineList(), k = allActive || mineActive ? null : curators.find(x => x.id === selKur[0]);
  chip(`<span class="e">${k ? '👤' : mineActive ? '♥' : '★'}</span>${esc(k ? kname(k) : mineActive ? t('optMine') : t('allShort'))}`, t('change'), () => { step = 'kur'; save(); render(); });
}
function render() {
  document.getElementById('step-art').hidden = step !== 'art';
  document.getElementById('step-place').hidden = step !== 'place';
  document.getElementById('step-kur').hidden = step !== 'kur';
  document.getElementById('step-wish').hidden = step !== 'main';
  drawSummary(); drawPlace(); drawTiles(); drawRadar(); drawOverlayInfo(); drawCurators(); drawList();
}
let resizeT; window.addEventListener('resize', () => { clearTimeout(resizeT); resizeT = setTimeout(drawRadar, 120); });
// Kopfzeilen-Höhe für das «klebende» Netz
(function headerH() {
  const hdr = document.getElementById('hdr');
  const set = () => document.documentElement.style.setProperty('--header-h', hdr.offsetHeight + 'px');
  set(); if ('ResizeObserver' in window) new ResizeObserver(set).observe(hdr); else window.addEventListener('resize', set);
})();
(function jumpBtn() {
  const btn = document.getElementById('jump'), list = document.getElementById('rest-list');
  btn.addEventListener('click', () => list.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  if ('IntersectionObserver' in window) new IntersectionObserver(([e]) => btn.classList.toggle('show', step === 'main' && !e.isIntersecting && window.scrollY > 200), { threshold: 0.05 }).observe(list);
})();
