...
const T = {
  de: {
    ...
    ignore: 'Ist mir egal', reactivate: 'Wieder werten', change: 'ändern', ratingsBy: 'Bewertungen von:', whole: 'ganz',
  ...
  en: {
    ...
    ignore: 'Skip this', reactivate: 'Count again', change: 'change', ratingsBy: 'Ratings by:', whole: 'all of',
...
let step = 'art'; try { step = localStorage.getItem(LS_STEP) || 'art'; } catch {} if (!['art', 'place', 'main'].includes(step)) step = 'art';
const MINE = 'mine';
...
function drawPlace() {
  ...
  for (const n of ids) {
    ...
    mk('path', { d: k.d }, g);
    const toggle = () => { kreise = on ? kreise.filter(x => x !== n) : [...kreise, n]; if (kreise.length === ids.length) kreise = []; step = 'main'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  all.addEventListener('click', () => { kreise = []; step = 'main'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); }); chips.appendChild(all);
...
function drawCurators() {
  ...
  const note = document.getElementById('kur-note');
  note.title = t('proto') + ' ' + t('protoHint');
  note.textContent = 'ⓘ';
}
...
function drawList() {
  ...
  rows.forEach(({ r, s }, idx) => {
    ...
    div.addEventListener('click', e => { if (e.target.closest('a,button')) return; selectedId = selectedId === r.id ? null : r.id; render(); if (selectedId) document.getElementById('step-wish').scrollIntoView({ behavior: 'smooth', block: 'start' }); });
...
<li><strong>Mir ist wichtig</strong> – Spinne einstellen, Unwichtiges auf «egal». Darunter wählst du, wessen Bewertungen zählen: alle, deine Liste oder eine einzelne Stimme.</li>
<li><strong>Deine Treffer</strong> – das Ranking, das zu deinem Abend passt. Oben in der Kopfzeile siehst du jederzeit deine Wahl und kannst sie ändern.</li>
</ol>
...
<li><strong>What matters to me</strong> – set the chart, skip what doesn't matter. Below it you choose whose ratings count: everyone, your list or a single voice.</li>
<li><strong>Your matches</strong> – the ranking that fits your evening. The header always shows your choices; tap them to change.</li>
</ol>
...
function drawSummary() {
  ...
  if (step === 'art') return;
  const a = document.createElement('button'); a.className = 'sum';
  a.innerHTML = `<span class="e">${artFilter ? (EMOJI[artFilter] || '🍽️') : '🎲'}</span>${esc(artFilter ? artName(artFilter) : t('surprise'))}`;
  a.title = t('change'); a.addEventListener('click', () => { step = 'art'; save(); render(); });
  box.appendChild(a);
  if (step === 'main') {
    const p = document.createElement('button'); p.className = 'sum';
    p.innerHTML = `<span class="e">📍</span>${esc(kreise.length ? `${t('kreis')} ${[...kreise].sort((x, y) => x - y).join(', ')}` : `${t('whole')} ${city}`)}`;
    p.title = t('change'); p.addEventListener('click', () => { step = 'place'; save(); render(); });
    box.appendChild(p);
  }
}
function render() {
  ...
  document.getElementById('step-list').hidden = step !== 'main';
