...
const T = {
  de: {
    ...
    ignore: 'Ist mir egal', reactivate: 'Wieder werten', change: 'ändern', ratingsBy: 'Bewertungen von:', whole: 'ganz', skip: 'Überspringen', allReset: 'Alles zurückgesetzt', allShort: 'Alle', avg: 'Durchschnitt aller {n} Stimmen',
  ...
  en: {
    ...
    ignore: 'Skip this', reactivate: 'Count again', change: 'change', ratingsBy: 'Ratings by:', whole: 'all of', skip: 'Skip', allReset: 'Everything reset', allShort: 'Everyone', avg: 'Average of all {n} voices',
...
function drawPlace() {
  ...
  for (const n of ids) {
    ...
    mk('path', { d: k.d }, g);
    const toggle = () => { kreise = on ? kreise.filter(x => x !== n) : [...kreise, n]; if (kreise.length === ids.length) kreise = []; step = 'kur'; save(); render(); window.scrollTo({ top: 0, behavior: 'smooth' }); };
